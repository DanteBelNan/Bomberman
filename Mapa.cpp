...
void Mapa::generarMapa() {
	...
	for (int i = 0; i < _filas; i++) {
		_mat[i] = new int[_columnas];
	}
	for (int i = 0; i < _filas; i++) { // 2 ES BLOQUE FIJO, 0 ES LIBRE Y 1 ROMPIBLE
	...
	_bf = new BloqueFijo[76];
	_bd = new BloqueDestruibles[_contRandom];
	int posD = 0, posF = 0;
	for (int i = 0; i < _filas; i++) {
		for (int j = 0; j < _columnas; j++) {
			if (_mat[i][j] == 1) {
				_bd[posD].setPosicionBloque(i, j);
				posD++;
			}
			else if (_mat[i][j] == 2) {
				_bf[posF].setPosicionBloque(i, j);
				posF++;
			}
		}
	}
